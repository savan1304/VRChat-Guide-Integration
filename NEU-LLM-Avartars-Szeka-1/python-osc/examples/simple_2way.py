...
if __name__ == "__main__":
    ...
    parser.add_argument("--serverport", type=int, default=9000, help="The port the OSC Server is listening on")
    parser.add_argument("--clientip", default="127.0.0.1", help="The ip of the OSC server")
    parser.add_argument("--clientport", type=int, default=9000, help="The port the OSC Client is listening on")
...
def random_values(client):        
    while True:
        for x in range(10):
            client.send_message("/1/fader2", 1)
            client.send_message("/1/fader1", 1)
            client.send_message("/1/xy1", 1, 1)
            time.sleep(.5)
